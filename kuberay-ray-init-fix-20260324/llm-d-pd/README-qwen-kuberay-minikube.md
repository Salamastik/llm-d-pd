...
Do not assume `vllm/vllm-openai:latest` or `ghcr.io/llm-d/llm-d-cpu:v0.5.0` includes `ray`.

Recent `vLLM` images may not ship `ray` as a default dependency, so the Ray head pod can fail with:

```text
ray: command not found
```

The demo values file works around this by installing `ray` from an `initContainer` into a shared volume mounted into the head and worker containers.
